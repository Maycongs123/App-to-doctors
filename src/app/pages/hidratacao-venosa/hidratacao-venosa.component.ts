...
export class HidratacaoVenosaComponent {
  ...
  ngOnInit(): void {
    ...
    this.medicamentosService.GetAll().subscribe((response: any) => {
      if (tipoAtendimento === 'Adulto') {
        this.medicacoes = response.filter(function (medicacao: any) {
          return (
            medicacao.medicamentoUso == 'Adulto' &&
            medicacao.tipo == 'Hidratação Venosa'
          );
        });
      }
      if (tipoAtendimento === 'Pediatrico') {
        this.medicacoes = response.filter(function (medicacao: any) {
          return (
            medicacao.medicamentoUso == 'Pediátrico' &&
            medicacao.tipo == 'Hidratação Venosa'
          );
        });
      }
  ...
  calcularHidratacaoVenosa() {   
    ...
    this.etapaSG5 = (this.valueSG5 / this.etapas).toLocaleString('pt-BR', {
      minimumFractionDigits: 2,
    });
    this.etapaNaCl20 = (this.valueNaCl20 / this.etapas).toLocaleString(
      'pt-BR',
      { minimumFractionDigits: 2 }
    );
    this.etapaKCl10 = (this.valueKCl10 / this.etapas).toLocaleString('pt-BR', {
      minimumFractionDigits: 2,
    });
    this.etapaNaCl20Tonico = (
      this.valueNaCl20Tonico / this.etapas
    ).toLocaleString('pt-BR', { minimumFractionDigits: 2 });

    this.taxaInfusaoBombaVenosa = (
      (this.valueSG5 + this.valueNaCl20 + this.valueKCl10) /
      24
    ).toLocaleString('pt-BR', { minimumFractionDigits: 2 });
    this.taxaInfusaoGotasMinVenosa = (
      (this.valueSG5 + this.valueNaCl20 + this.valueKCl10) /
      72
    ).toLocaleString('pt-BR', { minimumFractionDigits: 2 });
    this.taxaInfusaoMicrogotasMinVenosa = (
      (this.valueSG5 + this.valueNaCl20 + this.valueKCl10) /
      24
    ).toLocaleString('pt-BR', { minimumFractionDigits: 2 });

    this.taxaInfusaoBombaIsotonica = (
      (this.valueSG5 + this.valueNaCl20Tonico + this.valueKCl10) /
      24
    ).toLocaleString('pt-BR', { minimumFractionDigits: 2 });
    this.taxaInfusaoGotasMinIsotonica = (
      (this.valueSG5 + this.valueNaCl20Tonico + this.valueKCl10) /
      72
    ).toLocaleString('pt-BR', { minimumFractionDigits: 2 });
    this.taxaInfusaoMicrogotasMinIsotonica = (
      (this.valueSG5 + this.valueNaCl20Tonico + this.valueKCl10) /
      24
    ).toLocaleString('pt-BR', { minimumFractionDigits: 2 });
  }
